...
async function scene_2(give_or_no_give){
    if (check_inventory("food") && give_or_no_give){
        const index = player_data.inventory.indexOf("food");
...
async function scene_4(){
    if (!check_inventory("warm_clothes")) {damage_return(60,70,"You didn't have warm clothes during this cold and took a damage of ")}
    console.log("You had warm clothes in cold weather and took no damage")
}

async function scene_5(){
    if (!check_inventory("knife")) {damage_return(40,50,"You didn't have any weapon during a fight and took a damage of ")}
